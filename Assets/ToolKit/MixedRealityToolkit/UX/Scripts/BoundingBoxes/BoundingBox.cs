...
namespace MixedRealityToolkit.UX.BoundingBoxes
{
    ...
    public class BoundingBox : MonoBehaviour
    {
		...
		{
			int firstlevchild = 0;
			List<MeshFilter> firstlevfilters = new List<MeshFilter>();
			foreach (var child in target.GetComponentsInChildren<MeshFilter>())
			{

				if (child.transform.parent == target.transform)
				{
	//				Debug.Log(child);

					firstlevfilters.Add(child.GetComponent<MeshFilter>());
					firstlevchild++;
				}
			}
			MeshFilter[] meshFilters = new MeshFilter[firstlevchild];
			foreach (var filter in firstlevfilters)
			{
				meshFilters[firstlevfilters.IndexOf(filter)] = filter;
			}
